...
class llmHandler:
    llm_model: str
    system_prompt: str
    application_config: applicationParser
    behaviour_config: behaviourParser
    ...
    def create(cls, 
               llm_model: str, 
               system_prompt: str, 
               application_cfg: applicationParser, 
               behaviour_cfg: behaviourParser) -> "llmHandler":
    ...
    def craft_message_history(self, prompt: str) -> list:
        return [
            {"role": "system", "content": self.system_prompt},
            {"role": "user", "content": prompt}
        ]

    def invoke_llm_constrained(self, prompt: str, class_response: any, context: str) -> Optional[dict]:
        configuration = {'temperature': 0}
        grammar = class_response.model_json_schema(mode='serialization')
        history = self.craft_message_history(prompt)
    ...
    def _structured_output_call(self, prompt: str) -> Generator[tuple[dict | None, predicate], None, None]:
        behaviour_context = ""
        ...
        for pred in self.application_config.predicates:
            if pred.has_condition():
                if not self.predicate_condition_cache.skip_logic_solver(pred.conditions) and not pred.has_to_be_extracted():
                    continue

                self.predicate_condition_cache.update(pred.conditions, True)

            appl_mapping = behaviour_mapping.replace("{instructions}", f"{pred.prompt_description}")
            appl_mapping = appl_mapping.replace("{atom}", pred.predicate)
